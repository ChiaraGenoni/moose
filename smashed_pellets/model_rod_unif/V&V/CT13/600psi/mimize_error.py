...
for idx, (key, df) in enumerate(data_frames.items()):
    ...
    x_values = df['time']
    y_values = df['inlet-p']
    experimental_y_values = np.interp(x_values, data20['time'], data20['602_control_tube_1'])
...
plt.plot(data20['time'], data20['602_control_tube_1'], 'v', color='k', label = 'experimental results')
